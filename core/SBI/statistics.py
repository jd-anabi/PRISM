...
class SummaryStatistics:
    ...
    def get_summary_statistics(self, n: int = 1) -> torch.Tensor:
        """
        ...
        # static stats
        moments = self.__moments(self.x, 4)
        progress_bar.update()
        pdf_features = self.__pdf_features(self.x, n)
        progress_bar.update()

        # dynamic stats (time-domain)
        acf_at_lags = self.__acf_at_lags(self.x, n)
        progress_bar.update()
        zero_crossing_stats = self.__crossing_stats(self.x, self.dt, 0)
        progress_bar.update()
        cramer_crossing_stats = self.__crossing_stats(self.x, self.dt, pdf_features[:, 2])
        progress_bar.update()
        sample_entropy = self.__sample_entropy(self.x)
        progress_bar.update()

        # dynamic stats (frequency-domain)
        psd_peak_stats = self.__psd_peak_features(self.x, self.dt)
        progress_bar.update()
        binned_psd_pwr = self.__binned_psd_pwr(self.x, n, self.dt)
        progress_bar.update()

        # dynamic stats (phase-domain)
        analytic_signal_stats = self.__analytic_signal_stats(self.x)
        progress_bar.update()
        bicoherence = self.__mean_bicoherence(self.x, self.dt)
        progress_bar.update()
    ...
    # STATIC STATISTICS
    @staticmethod
    def __moments(x: torch.Tensor, order: int) -> torch.Tensor:
        """
    ...
    @staticmethod
    def __pdf_features(x: torch.Tensor, nbins: int) -> torch.Tensor:
        """
    ...
    @staticmethod
    def __acf_at_lags(x: torch.Tensor, nlags: int) -> torch.Tensor:
        """
    ...
    @staticmethod
    def __crossing_stats(x: torch.Tensor, dt: float, boundary: Union[float, torch.Tensor] = 0) -> torch.Tensor:
        """
    ...
    @staticmethod
    def __sample_entropy(x: torch.Tensor, m: int = 2, r: float = None, downsample: int = 100) -> torch.Tensor:
        """
    ...
    @staticmethod
    def __psd_peak_features(x: torch.Tensor, dt: float) -> torch.Tensor:
        """
    ...
    @staticmethod
    def __binned_psd_pwr(x: torch.Tensor, nbins: int, dt: float, norm: bool = False) -> torch.Tensor:
        """
        ...
        xf = torch.fft.rfft(x - torch.mean(x, dim=1, keepdim=True), dim=1)
        psd = torch.abs(xf) ** 2 * dt / (xf.shape[-1])
    ...
    @staticmethod
    def __mean_bicoherence(x: torch.Tensor, dt: float, nperseg: int = 256, step: int = 128, nfft: int = 512) -> torch.Tensor:
        """

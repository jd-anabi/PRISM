...
from core.Simulator import nd_simulator
from core.SBI.Priors import prior

class NDPrior(prior.Prior):
    ...
    def _local_map(t: torch.Tensor, stable_params: list, batch_size: int, n_params: int, n_max: int, step: float, segs: int, steady: bool) -> list:
        ...
        with torch.no_grad():
            while len(queue) != 0 and len(accepted_params) <= n_max:
                ...
                sim = bp_simulator.NDSimulator(thetas, force, inits, t, segs=segs, batch_size=batch_size, device=device)

...
from core.Simulator import nd_nadrowski_simulator
from core.SBI.Priors import prior

class NDNadrowskiPrior(prior.Prior):
    ...
    def _global_map(self, t: torch.Tensor, n_params: int, prior_bounds: list[tuple], segs: int,
        ...
        with torch.no_grad():
            for i in range(num_iterations - 1):
                ...
                sim = nd_nadrowski_simulator.NDNadrowskiSimulator(curr_thetas, force, inits_tensor, t, segs=segs, batch_size=curr_batch_size, device=self.device)
    ...
    def _local_map(t: torch.Tensor, stable_params: list, batch_size: int, n_params: int,
        ...
        with torch.no_grad():
            while len(queue) != 0 and len(accepted_params) <= n_max:
                ...
                sim = nd_nadrowski_simulator.NDNadrowskiSimulator(thetas, force, inits, t, segs=segs, batch_size=batch_size, device=device)

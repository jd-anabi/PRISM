...
from core.Models import model, model_steady
from core.Simulator import simulator

class DimSimulator(simulator.Simulator):
    ...
    def _set_up_model(self):
        try:
            if self._params.shape[-1] != 36:
                self.inits = self.inits[:, :4]
                self.sde = model_steady.BPModelSteady(*torch.unbind(self._params, dim=1), self._force, batch_size=self._batch_size, device=self._device, dtype=self._dtype)
            else:
                if torch.all(self._params[:, 3]):
                    self.sde = model.BPModel(*torch.unbind(self._params, dim=1), self._force, batch_size=self._batch_size, device=self._device, dtype=self._dtype)
                else:

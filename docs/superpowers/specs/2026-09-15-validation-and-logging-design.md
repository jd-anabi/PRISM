...
| 4 | T3 | §2.4's chi probe-count pin describes `test_an_experimental_observation_records_its_own_length_and_drive_frequencies` as building three probes with a manifest of 200 samples; that test's chi leg drives two probes on 50-sample recordings. The caller pins land there with the test's own numbers (the caller's `chi_n_freqs` stays `config.CHI_N_FREQS`, `n_obs` stays None, no `chi_obs_freqs`; the manifest records 2 probes and 50 samples). The three-probe case is the `build_experiment_observation` leg of `test_every_public_entry_leaves_the_callers_config_untouched`, so §8.3's `test_a_bench_chi_observation_does_not_change_the_configs_probe_count` is not a separate test. | The pin is about the caller's config, not the probe count; rebuilding a passing leg to match a miscounted description changes nothing it checks. |
| 5 | T7 | §3.4's order "numeric knobs, then the name, then the files" holds in the two compositions only; `build_prior`, `build_posterior`, `validate_calibration` and `tsnpe_round` keep the name check first. The recording refusal is `require_file(<field>, p, "recording")`, not a stage-specific sentence. The drive rules run at the head of the forced builder, after the recordings are hashed and loaded but before any conversion, lock-in or statistic. `analysis.gen_cal_data` refuses `cal_n_scales` below 1 with `require_at_least` instead of `int()`. §1.3's optional tidy-up of the `or` fallbacks and clamps is not done. | A pin requires the round's name check first on every path (`store.asked == [("posterior", "")] * 4`). One sentence per recording field is shared with the Infer tab's click. Loading a recording costs no simulation. A bare `int` of 0 is still spread over one batch downstream, and `sbc_repeats` would keep the clamp. The stages now pass resolved numbers, and direct callers of those helpers keep their fallbacks. |
